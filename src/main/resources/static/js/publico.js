...
Pages.inicio = async () => {
    ...
    try {
      ...
      <div class="card" style="width:290px;position:relative">
        <div style="font-weight:bold;color:#831843;font-size:18px">${p.empresa.nombre}</div>
        <div style="font-size:14px;color:#4c0519;opacity:.7;margin-bottom:10px">
          ${p.descripcion.substring(0, 50)}…
        </div>
        <span style="font-weight:900;color:#4c0519;font-size:18px;display:block;margin-bottom:16px">
          ${p.moneda.codigo} ${Number(p.salario).toLocaleString('es-CR')}
        </span>
        <a class="btn-detail" onclick="Router.go('/buscar')"
           style="display:block;text-align:center;padding:10px;background:transparent;
                  color:#831843;border:2px solid #831843;border-radius:8px;
                  text-decoration:none;font-size:13px;font-weight:800;cursor:pointer">
          Ver detalle
        </a>
        <div class="tooltip-spa" style="display:none;position:absolute;top:95%;left:0;
             background:rgba(255,255,255,.85);backdrop-filter:blur(12px);
             border:1px solid rgba(131,24,67,.3);padding:20px;border-radius:12px;
             z-index:100;width:100%;box-shadow:0 15px 30px rgba(76,5,25,.15)">
    ...
    `).join('');
      ...
      <div class="container">
        ...
        <p class="subtitle" style="color:#9f1239;font-size:14px;margin-bottom:30px;
           text-transform:uppercase;letter-spacing:.5px">Últimos 5 puestos públicos</p>
        <div style="display:flex;flex-wrap:wrap;gap:20px">${cards}</div>
    ...
    `);
        ...
        document.querySelectorAll('.card').forEach(card => {
            const tip = card.querySelector('.tooltip-spa');
    ...
    <div style="display:flex;justify-content:center;align-items:center;min-height:70vh">
      <div class="auth-box narrow">
        ...
        <div class="auth-links" style="margin-top:20px">
          <a onclick="Router.go('/registro/empresa')" style="cursor:pointer;color:#831843">Registrar Empresa</a>
          <span class="divider">|</span>
          <a onclick="Router.go('/registro/oferente')" style="cursor:pointer;color:#831843">Registrar Oferente</a>
          <br><br>
          <a onclick="Router.go('/')" style="cursor:pointer;color:#831843">← Volver al inicio</a>
        </div>
      </div>
    </div>
  `);
    ...
    try {
      ...
      <div class="container-wide" style="padding:40px 10%;max-width:960px">
        ...
        <div style="margin-bottom:16px">
          ...
          <select id="b-moneda" style="padding:8px 12px;border:1px solid #fecdd3;border-radius:8px;
...
function _renderResultados(puestos, container) {
    ...
    <table>
      <thead>
        <tr>
          <th>Empresa</th><th>Descripción</th><th>Salario</th>
          <th>Moneda</th><th>Características</th>
        </tr>
      </thead>
      <tbody>
        ${puestos.map(p => `
...
Pages.registroEmpresa = () => {
    ...
    <div style="display:flex;justify-content:center;padding:40px">
      <div class="auth-box">
        ...
        <button id="re-btn" class="btn btn-primary" style="width:100%;margin-top:22px">Registrar Empresa</button>
        <div class="auth-links" style="margin-top:12px">
          <a onclick="Router.go('/login')" style="cursor:pointer;color:#831843">Ya tengo cuenta → Iniciar sesión</a>
        </div>
      </div>
    </div>
  `);
...
Pages.registroOferente = () => {
    ...
    <div style="display:flex;justify-content:center;padding:40px">
      <div class="auth-box">
        ...
        <button id="ro-btn" class="btn btn-primary" style="width:100%;margin-top:22px">Registrar Oferente</button>
        <div class="auth-links" style="margin-top:12px">
          <a onclick="Router.go('/login')" style="cursor:pointer;color:#831843">Ya tengo cuenta → Iniciar sesión</a>
        </div>
      </div>
