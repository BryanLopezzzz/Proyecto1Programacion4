...
export default function BuscarPuestos() {
  ...
  useEffect(() => { getCaracteristicas().then(setRaices).catch(() => {}) }, [])
  ...
  const buscar = async () => {
    ...
    setResultados(res || []); setBuscado(true)
  }

  const limpiar = () => { setSeleccionadas([]); setResultados([]); setBuscado(false) }

  return (
    <div className="container-wide" style={{ padding: '40px 10%', maxWidth: 960 }}>
          ...
          {p.coincidencias && p.coincidencias.length > 0 && (
            <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {p.coincidencias.map(d => (
                <span key={d.caracteristica} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 10, background: estadoColor[d.estado], border: '1px solid #d1d5db' }}>
